...
test_rgb_mixer:
	...
	COCOTB_TEST_MODULES=test.test_rgb_mixer vvp -M $(shell cocotb-config --lib-dir) -m $(shell cocotb-config --lib-name vpi icarus) sim_build/sim.vvp
...
test_encoder:
	...
	COCOTB_TEST_MODULES=test.test_encoder vvp -M $(shell cocotb-config --lib-dir) -m $(shell cocotb-config --lib-name vpi icarus) sim_build/sim.vvp
	! grep failure results.xml
...
test_pwm:
	...
	COCOTB_TEST_MODULES=test.test_pwm vvp -M $(shell cocotb-config --lib-dir) -m $(shell cocotb-config --lib-name vpi icarus) sim_build/sim.vvp
	! grep failure results.xml
...
test_debounce:
	...
	COCOTB_TEST_MODULES=test.test_debounce vvp -M $(shell cocotb-config --lib-dir) -m $(shell cocotb-config --lib-name vpi icarus) sim_build/sim.vvp
	! grep failure results.xml
